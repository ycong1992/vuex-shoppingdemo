...
const mutations = {
  ...
  // Test:将购物车清空的商品还原到仓库中
  [PRODUCTS.RESET_PRODUCTS] (state, {items} ) {
    items.forEach(element => {
      const product = state.all.find(product => product.id === element.id);
      product.inventory += parseInt(element.quantity);
    });
  }
}
